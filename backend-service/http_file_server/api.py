...
def checkVerifyCode():
    ...
    result, sessionID, errorMsg = account.checkVerifyCode(
        phoneNo, verifyCode, deviceID)
...
def sendBindVerifyCode():
    ...
    result, errorMsg = account.sendBindVerifyCode(
        phoneNo, deviceID, content['sessionID'])
    return json.dumps({"result": result, "errorMsg": errorMsg})
...
def checkBindVerifyCode():
    ...
    result, qrcode, bindingkey, errorMsg = account.checkBindVerifyCode(
        phoneNo, verifyCode, deviceID, sessionID)
...
if __name__ == '__main__':
    app.run(debug=True, host="127.0.0.1", port=12000, use_reloader=False)
